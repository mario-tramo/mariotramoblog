...
export default function ScrollCarousel({
	...
	return (
		<div ref={wrapperRef} className={cn('group/carousel relative', className)}>
			...
			{/* Soft edge cue: keeps the next partial card recognizable while
			    suggesting that more content is available to the right. */}
			<div
				aria-hidden="true"
				className={cn(
					'pointer-events-none absolute inset-y-0 right-0 z-[5] w-8 bg-gradient-to-l from-canvas/55 via-canvas/15 to-transparent backdrop-blur-[1px] transition-opacity duration-300 sm:w-12',
					'[mask-image:linear-gradient(to_left,black_8%,black_24%,transparent_100%)]',
					'[-webkit-mask-image:linear-gradient(to_left,black_8%,black_24%,transparent_100%)]',
			...
			{showArrows && (
				<>
					<button
						...
						onClick={() => scroll('left')}
						className={cn(
							'absolute top-1/2 left-1 z-10 hidden -translate-y-1/2 place-items-center rounded-full border border-white/20 bg-[rgba(5,15,26,0.75)] text-ink transition duration-[180ms] hover:bg-brand hover:text-white sm:grid',
							'size-8',
					...
					<button
						...
						className={cn(
							'absolute top-1/2 right-1 z-10 hidden -translate-y-1/2 place-items-center rounded-full border border-white/20 bg-[rgba(5,15,26,0.75)] text-ink transition duration-[180ms] hover:bg-brand hover:text-white sm:grid',
							'size-8',
					...
					{Array.from({ length: totalPages }).map((_, i) => (
						<button
							...
							className={cn(
								...
								i === currentPage
									? 'w-[18px] bg-brand'
									: 'w-[6px] bg-ink/20 hover:bg-ink/40',
							)}
